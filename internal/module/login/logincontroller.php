...
$userGroupResult = $objLogin->getUserGroup($username);
$group = "";
if (($userGroupResult->num_rows) > 0) {
    $rowData = $userGroupResult->fetch_assoc();
    $group = $rowData['group_name'];
}
if ($group != "Blocked") {
    ...
    $msg = "You can't access the system. Your account has blocked.";
    $msg1 = base64_encode($msg); //Encoding Mechanism
    header("Location:index.php?msg=$msg1"); //Redirection n passing data
    //Through URL
}
